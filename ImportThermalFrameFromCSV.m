%% Import thermal data frame from text (csv) file.
function [Frame, Time] = ImportThermalFrameFromCSV(filename)

    if nargin == 0
        filename = 'D:\NPD\PCI Thermal Data\Rec_000089_Heat7_Medial\Rec_000089_Heat7_Medial_300.csv';
    end
    ...
    % Read the header and extract the time value
    fgets(fileID);
    fgets(fileID);
    strTime = fgets(fileID); % String containing "Time = DAY:HOUR:MIN:SEC", where days are measured from start of year
    strTimeVals = strsplit(strTime(8:end), ':');
    Time = 60*60*str2double(strTimeVals(2)) + 60*str2double(strTimeVals(3)) + str2double(strTimeVals(4));
    fclose(fileID);
